...
MUTANTS = [
    ...
    dict(name="let an assignment stop early with damage left to give (465.2.c)",
         file="engine/combat.py",
         find="    if total < pool and pending_targets(s, defenders, assignment):",
         repl="    if False:",
    ...
    dict(name="ignore Tank when choosing what may be assigned damage (815.1.c.2)",
         file="engine/combat.py",
         find='    tanks = [u for u in pending if u["tank"]]',
         repl="    tanks = []",
         expect="only legal assignment until it has its lethal"),
    dict(name="ignore Backline when choosing what may be assigned damage (826.4.b)",
         file="engine/combat.py",
         find='    plain = [u for u in pending if not u["backline"]]\n    return plain if plain else pending',
         repl="    return pending",
         expect="the plain units become legal"),
    dict(name="stop refusing an assignment that jumps a Tank (815.1.c.2)",
         file="engine/combat.py",
         find='    jumped([u for u in live if u["tank"]], [u for u in live if not u["tank"]],',
         repl='    (lambda *a: None)([u for u in live if u["tank"]], [u for u in live if not u["tank"]],',
         expect="jumping a Tank is refused"),
    dict(name="stop refusing an assignment that jumps to Backline (826.4.b)",
         file="engine/combat.py",
         find='    jumped([u for u in live if not u["backline"]], [u for u in live if u["backline"]],',
         repl='    (lambda *a: None)([u for u in live if not u["backline"]], [u for u in live if u["backline"]],',
