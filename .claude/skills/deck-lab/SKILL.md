...
Then, per seat:
    ...
    do 'beginturn 0; cast 0 "Stellacorn Herder" base:0; smove u9 bf:0'

`do` takes several actions separated by `;`, applies them in order, and **stops at the
first refusal without applying anything after it**. Batch a whole turn; the round trip
is the expensive part.

### The rules of playing honestly

- **Decide from the seat view.** Run `state --seat N` and choose from what it shows.
  You cannot un-know the other hand, but you can refuse to consult it — and every
  decision is logged where the discrepancy would show.
