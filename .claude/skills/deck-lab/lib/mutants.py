...
MUTANTS = [
    ...
    dict(name="stop refusing the bracketed words CR 805-829 does not define",
         file="engine/dsl/schema.py",
         find="    if isinstance(name, str) and name in NOT_KEYWORDS:",
         repl="    if False:",
    ...
    dict(name="allow a cost reduction with no floor",
         file="engine/dsl/schema.py",
         find='    if name in ("modify_cost", "give_might") and negative and "min" not in node:',
         repl="    if False:",
    ...
    # ---- the review's findings, each with the check that now catches it ---
    dict(name="validate the first clause and skip the rest",
         file="engine/dsl/schema.py",
         find="    for i, clause in enumerate(clauses):",
         repl="    for i, clause in enumerate(clauses[:1]):",
         expect="caught at index 3"),
    dict(name="let a table lookup fall through to the backstop, unguarded",
         file="engine/dsl/schema.py",
         find="    atom = table.get(name) if isinstance(name, str) else None",
         repl="    atom = table.get(name)",
         expect="handled by a named check, not by a raise or the backstop"),
    dict(name="catch the raise in the backstop instead of guarding for it",
         file="engine/dsl/schema.py",
         find='    if not isinstance(kind, str) or kind not in _ABILITIES:',
         repl="    if kind not in _ABILITIES:",
         expect="handled by a named check, not by a raise or the backstop"),
    dict(name="walk a cycle until the stack runs out",
         file="engine/dsl/schema.py",
         find="    def too_deep(self, path):\n        if len(path) <= MAX_PATH:",
         repl="    def too_deep(self, path):\n        if True:",
         expect="reported as too deep, not raised"),
    dict(name="slug a card name the corpus regex cannot read",
         file="engine/dsl/library.py",
         find="    if not isinstance(name, str):\n        return \"\"",
         repl="    if False:\n        return \"\"",
         expect="a hostile script is one bad row, not an exception"),
    dict(name="stop requiring the clauses to reconstruct the printed text",
         file="engine/dsl/schema.py",
         find="        if joined != want:",
         repl="        if False:",
         expect="breaks the partition rather than raising coverage"),
    dict(name="accept a selector that contradicts itself",
         file="engine/dsl/schema.py",
         find="        if a in value and b in value and value.get(a) is not False:",
         repl="        if False:",
         expect="a selector that contradicts itself is refused"),
    dict(name="let two abilities claim one id",
         file="engine/dsl/schema.py",
         find="        if ident in seen:",
         repl="        if False:",
         expect="an id claimed by two abilities is refused"),
    dict(name="let a `ref` read a bind nothing establishes",
         file="engine/dsl/schema.py",
         find="        if name not in w.binds:",
         repl="        if False:",
         expect="a `ref` to a bind nothing establishes is refused"),
    dict(name="allow a negative count on a cost or a draw",
         file="engine/dsl/schema.py",
         find="            if value < 0:\n"
              '                w.add("bad_value", path, "expected a count of zero or more",',
         repl="            if False:\n"
              '                w.add("bad_value", path, "expected a count of zero or more",',
         expect="a negative draw is refused"),
    dict(name="clamp only cost reductions, not Might modifiers",
         file="engine/dsl/schema.py",
         find='    if name in ("modify_cost", "give_might") and negative and "min" not in node:',
         repl='    if name in ("modify_cost",) and negative and "min" not in node:',
         expect="a negative Might modifier with no floor is refused"),
    dict(name="let a programmatically selected set be a Target (355.10)",
         file="engine/dsl/schema.py",
         find='        if value.get("all") is True:',
         repl="        if False:",
         expect="`all` and `targets` cannot both hold"),
    dict(name="let an object in a hand or a deck be a Target (128)",
         file="engine/dsl/schema.py",
         find='        if value.get("zone") in ("hand", "deck", "top_of_deck"):',
         repl="        if False:",
         expect="an object in a non-public zone is not a Target"),
    dict(name="report a keyword parameter without the set it came from",
         file="engine/dsl/schema.py",
         find='              expected=[k for k, v in KEYWORDS.items() if not v[2]])',
         repl="              )",
         expect="carries the set it should have come from"),
    dict(name="credit a token filter as though it created a token",
         file="engine/dsl/schema.py",
         find="    if w.creating:\n        _compose(\"selector.token\", path, w)",
         repl='    if True:\n        _compose("selector.token", path, w)',
         expect="a token spec used as a FILTER does not credit token creation"),
    dict(name="slug a card by the spelling it was handed",
         file="engine/dsl/library.py",
         find='    card = cards.find(name)\n'
              '    return _SLUG_STRIP.sub("-", ((card["name"] if card else name) or "").lower()).strip("-")',
         repl='    return _SLUG_STRIP.sub("-", (name or "").lower()).strip("-")',
         expect="a bare champion name reaches the same script as its full name"),
    dict(name="count a deck's distinct cards without its legend",
         file="deckfile.py",
         find="        for name in [deck.legend, deck.chosen_champion]:",
         repl="        for name in []:",
         expect="readiness counts the deck's distinct cards"),
    dict(name="hash every script to the same constant",
         file="engine/dsl/schema.py",
         find='    payload = json.dumps(body(doc), sort_keys=True, separators=(",", ":"),',
         repl='    payload = json.dumps({}, sort_keys=True, separators=(",", ":"),',
         expect="two different scripts do not share a version"),
    dict(name="score word coverage over the implemented words alone",
         file="engine/dsl/library.py",
         find='        "word_coverage": (done / words) if words else 0.0,',
         repl='        "word_coverage": 1.0 if done else 0.0,',
         expect="leaves the WORD ratio where it was"),
    dict(name="give `frequency` a second home inside the trigger",
         file="engine/dsl/schema.py",
         find='    "condition": "condition", "keyword": "keyword",\n'
              '    "seat": "seat", "state": "state", "zone": "zone",\n}',
         repl='    "condition": "condition", "keyword": "keyword",\n'
              '    "seat": "seat", "state": "state", "zone": "zone",\n'
              '    "frequency": "frequency",\n}',
         expect="a frequency inside a trigger is refused"),
    dict(name="let a trigger listen for a window as though it were an event",
         file="engine/dsl/schema.py",
         find="        if isinstance(name, str) and name in NOT_EVENTS:",
         repl="        if False:",
         expect="it is a window, not an event"),
    dict(name="leave a delayed ability crediting nothing",
         file="engine/dsl/schema.py",
         find='    "delayed": (("trigger", "delayed_next"),),',
         repl='    "delayed": (),',
         expect="credits the census's `delayed_next`"),
    ...
    dict(name="report every active atom as exercised, whatever the scripts do",
         file="engine/dsl/library.py",
         find='        "atoms_exercised": len(active & exercised),',
         repl='        "atoms_exercised": len(active),',
         expect="a library of one script reports only the atoms that one script reaches"),
