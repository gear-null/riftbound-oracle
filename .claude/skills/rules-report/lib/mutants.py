...
MUTANTS = [
    ...
    # ---- the portable-copy button ----------------------------------------
    # One masthead element, three states. The failure this design exists to
    # make impossible is the LINK form appearing without the file it names —
    # a button that saves something nothing wrote. Each state has a mutant.
    dict(name="render the button as a link before any export has run",
         file="render_report.py",
         find="    {portable_note(stem)}\n  </div>\n</header>",
         repl='    {portable_link(f"{stem}.portable.html")}\n  </div>\n</header>',
         expect="plain render's control is the NOT-BUILT note"),
    dict(name="write the link even when the export refused",
         file="export_report.py",
         find="        return _must_replace(html, PORTABLE_SLOT, portable_note(stem),\n                             \"the masthead's portable-copy slot\"), None, str(err)",
         repl="        return _must_replace(html, PORTABLE_SLOT, portable_link(f\"{stem}.portable.html\"),\n                             \"the masthead's portable-copy slot\"), None, str(err)",
         expect="export refuses, the control is the note and there is NO link"),
    dict(name="leave the button as a note after a successful export",
         file="export_report.py",
         find="    upgraded = _must_replace(html, PORTABLE_SLOT, portable_link(f\"{stem}.portable.html\"),",
         repl="    upgraded = _must_replace(html, PORTABLE_SLOT, portable_note(stem),",
         expect="control becomes a link to the sibling file"),
    dict(name="let the portable copy keep a link to a sibling it travelled without",
         file="export_report.py",
         find="    out = _must_replace(out, PORTABLE_SLOT, PORTABLE_BADGE, \"the portable-copy control\")",
         repl="    pass",
         expect="returns a portable document on success"),
    dict(name="drop `download`, so the button navigates instead of saving",
         file="render_report.py",
         find="            f'download=\"{portable_name}\" title=\"One self-contained file: every cited '",
         repl="            f'title=\"One self-contained file: every cited '",
         expect="carries `download`"),
    dict(name="match the slot by its not-built form only, so re-export refuses",
         file="render_report.py",
         find="PORTABLE_SLOT = re.compile(r'<(a|span) class=\"portable[^\"]*\" id=\"portable\"[^>]*>.*?</\\1>', re.S)",
         repl="PORTABLE_SLOT = re.compile(r'<(span) class=\"portable is-unbuilt\" id=\"portable\"[^>]*>.*?</\\1>', re.S)",
         expect="accepts a report whose control is already the link form"),
    dict(name="forget the note's stem, so it names no file",
         file="render_report.py",
         find="&mdash; <code>rules_cli.py export {stem}.html</code></span>')",
         repl="&mdash; <code>rules_cli.py export</code></span>')",
         expect="names the export command for THIS file"),
    dict(name="stop asserting the badge arrived in the portable file",
         file="export_report.py",
         find="        (PORTABLE_BADGE, \"the portable-copy badge\"),\n",
         repl="",
         expect="badge removed is detectable"),
    dict(name="drop the control from the primer masthead",
         file="render_primer.py",
         find="    {portable_note(stem)}\n  </div>\n</header>",
         repl="  </div>\n</header>",
         expect="PRIMER carries the same portable-copy control"),    dict(name="skip building the portable copy in `report`, leaving the note",
         file="rules_cli.py",
         find="    _attach_portable_copy(out)\n",
         repl="",
         expect="said which it did"),
    dict(name="write the report's link but never the portable file it points at",
         file="rules_cli.py",
         find="    _write_atomically(portable_path, lambda fh: fh.write(portable_html))\n    _write_atomically(out, lambda fh: fh.write(report_html))",
         repl="    _write_atomically(out, lambda fh: fh.write(report_html))",
         expect="IF AND ONLY IF the portable sibling exists"),
]
