...
MUTANTS = [
    ...
    dict(name="score a missing mutation battery as full coverage",
         file="selftest.py",
         find='        return "  (mutants.py unreadable — cannot say how many checks can fail)"',
         repl='        expects = []',
         expect="derived from mutants.py, not asserted"),
    dict(name="count only PROVEN checks, so an untested one cannot lower the ratio",
         file="selftest.py",
         find="    names = set(NAMES)",
         repl="    names = {n for n in NAMES if any(e in n for e in expects)}",
