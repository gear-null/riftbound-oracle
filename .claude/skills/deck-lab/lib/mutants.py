...
MUTANTS = [
    ...
    dict(name="shuffle the Chosen Champion into the Main Deck",
         file="deckfile.py",
         find="            if self.chosen_champion and _same(name, self.chosen_champion):\n                copies -= 1",
         repl="            if False:\n                copies -= 1",
    ...
    dict(name="draw from an empty deck in silence",
         file="table.py",
         find='                self.note(f"seat {seat} BURNS OUT — no cards left to draw (431)")',
         repl="                pass",
         expect="Burn Out"),
